...
# Euler module ids worth probing: installer/markets/liquidation/governance/exec/
# swap + the single-proxy eToken/dToken module implementations.
...
def discover_facet_module_addresses(onchain, address, abi=None, *, chain="ethereum", eth_call=None):
    ...
    # 1) EIP-2535 Diamond loupe: facetAddresses() -> address[]
    for a in _decode_address_array(_call(address, _selector("facetAddresses()"))):
        _add(a)

    # 2) Euler-style module dispatcher: moduleIdToImplementation/Proxy(uint256)
