...
with an Etherscan eth_call fallback when no node RPC is available.

Pure logic with fakes -- no network. This is the fix for the Euler visibility gap
(dispatcher surfaced, module code never fetched).
"""
from backend.core.source_fetcher import (
    ...
    _selector,
    discover_facet_module_addresses,
...
def test_decode_address_array_roundtrip():
    got = [g.lower() for g in _decode_address_array(_enc_addr_array([FACET_A, FACET_B]))]
    assert got == [FACET_A, FACET_B]
...
def test_discover_via_etherscan_fallback_when_rpc_down():
    table = {SEL_FACETS: _enc_addr_array([FACET_A]),
             SEL_MODIMPL + _word(500000): "0x" + _addr_word(FACET_B)}
