"""Interprocedural / cross-function reentrancy (v0.5).

Regression for the Rari/Fuse CEther miss: the external send lives in a helper
(doTransferOut) one hop away from the function that writes state (borrowFresh).
The single-body v0.4 detector saw no `.call{}` in borrowFresh and stayed silent.
...
def _reentrancy(src):
    return [f for f in ReentrancyDetector().run(_ctx(src))
            if f.evidence.get("bug_class") == "reentrancy"]


# Rari/Fuse shape: external send hidden in an internal helper, state written after.
RARI = """contract Fuse {
...
# CEI-correct: effects BEFORE the (interprocedural) interaction -> no finding.
...
# Helper sends but caller writes NOTHING after -> no finding.
SAFE_NOWRITE = """contract Safe2 {
  function doTransferOut(address payable to, uint amt) internal {
    to.transfer(amt);
  }
  function pay(address payable to, uint amt) internal {
    uint x = amt;
    doTransferOut(to, amt);
  }
}"""


def test_interprocedural_cei_detected():
    findings = _reentrancy(RARI)
    assert findings, "interprocedural CEI (Rari/Fuse) should be flagged"
    inter = [f for f in findings if f.evidence.get("interprocedural")]
    assert inter, "should mark the helper-hop case as interprocedural"
    assert any("borrowFresh" in f.affected_functions for f in inter)
